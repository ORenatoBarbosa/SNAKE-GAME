...
class Game:
    ...
    def __init__(self):
        self.window = Tk()
        self.canvas = Canvas(self.window, bg='Orange', width=width, heigh=heigh)
        self.canvas.pack()
        ...
        f = square(random.randint(grid_size, (width / grid_size)) * grid_size - grid_size,
                   random.randint(grid_size, (heigh / grid_size)) * grid_size - grid_size, 'purple')
        ...
        while (True):
            ...
            for i in range(2, len(self.snake)):
                if (counter < 1):
                    ...
                    showinfo(title="FIM DO JOGO", message="GAME OVER!")
                    exit()
